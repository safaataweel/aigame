...
player_colors = {
    1: (0,0,0)  ,  # black color for player 1
    2:(255,255,255) # white color for player 2
}
...
def print_winner(player):
    ...
    player_color = "Black" if player == 1 else "White"
        ...
        for col in range(BOARD_COLS - 4):
            ...
            for i in range(5):
                if board[row + i][col + i] == player:
                    ...
                    if i == 2 and board[row + i + 1][col + i + 1] == player and board[row + i + 2][col + i + 2] == player:
                        empty_cell = (row + i, col + i)
                        break
                    elif  i == 3 and board[row + i + 1][col + i + 1] == player:
                        empty_cell = (row + i, col + i)
                        break
                    if i == 1 and board[row + i - 1][col + i - 1] == player and board[row + i + 1][col + i + 1] == player:
                        empty_cell = (row + i, col + i)
                        break
                    else:
                        empty_cell = None
...
draw_lines()
player = 1  # Start with the black player(human)
game_over = False
running = True
...
while running:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
                ...
                if available_square(clicked_row, clicked_col) and is_legal_move(clicked_row, clicked_col):
                    ...
                    if check_win(1):
                        ...
                        game_over = True
                    player = 2  # Switch to the computer player
        if player == 2:
    ...
    if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_r:
            restart()
            player = 1 # Start with the black player
            game_over = False
